...
static void read(struct afb_req request) {
  ...
  if (args) {
      if (!json_object_object_get_ex(args, "button_id", &val)) {
  ...
  ret = obj.read(json_object_object_get_ex(args, "button_id", &val) ? static_cast<rover_button_id>(json_object_get_int(val)) : static_cast<rover_button_id>(0),
      _var_button_state);
  if (ret) {
